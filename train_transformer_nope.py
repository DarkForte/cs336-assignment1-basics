...
def main():
    parser = argparse.ArgumentParser(description="Train Transformer without Positional Embeddings")
    ...
    # Initialize the TransformerNoPE model (no positional embeddings)
    transformer_model = transformer_blocks.TransformerNoPE(
    ...
    try:
        ...
        final_checkpoint_path = "transformer_nope_final.ckpt"
        ...
        plt.ylabel('Loss')
        plt.title('Training Loss vs. Tokens Processed (No Positional Embeddings)')
        plt.grid(True)
        plt.yscale('log')
        plt.legend()
        plt.savefig('transformer_nope_learning_curve.png')
        plt.close()
